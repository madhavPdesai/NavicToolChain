! a coprocessor isr model for the case when an interrupt handler
...
coprocessor_isr:
   ...
   ! Step 0
   !    Save the PSR
   set coprocessor_isr_ibuffer, %l6
   mov %psr, %l7
   st %l7, [%l6]          ! Save PSR
   ...
   !    mode.
   set coprocessor_isr_intr_active, %l6
   ld [%l6], %l7
   cmp %l7, %g0
   be normal_exec
   nop
